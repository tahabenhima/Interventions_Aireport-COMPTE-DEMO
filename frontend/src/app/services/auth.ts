...
export class AuthService {
  ...
  login(pseudoname: string, motDePass: string) {
    return this.http.post<LoginResponse>(`${this.apiUrl}/login`, { pseudoname, motDePass })
      .pipe(
        tap(res => {
          const normalizedRole = (res.role ?? '').toLowerCase() as LoginResponse['role'];
          const normalizedUser: LoginResponse = { ...res, role: normalizedRole };
  ...
  private readStored(): LoginResponse | null {
    ...
    try {
      ...
      const normalizedRole = (parsed.role ?? '').toLowerCase() as LoginResponse['role'];
